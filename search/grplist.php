...
if($cnt==0)
{	echo "<br/><b><big>sob sob we don't have a single group sob sob :( </big></b>";	}
else
{
	echo "<div class='ex'><b><big>Group List....</big></b><br/>";
	while($row = mysql_fetch_array($result))
	{	$qry3="select users.id from users,groups,bel_user_grp where users.id=bel_user_grp.userid and bel_user_grp.groupid=groups.id and groups.id=$row[0] and users.id=$uid";
		$res3=mysql_query($qry3);
		$cnt3=mysql_num_rows($res3);
		if($cnt3==1){	echo "<img src='tick.png' alt='tick' width='3%' height='3%' align='top' />";	}
		echo "<a href='grphome.php?id=$row[0]'>$row[1]</a><br/>";	
	}
	echo "</div>";
}
?>
